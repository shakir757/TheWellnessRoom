...
class ProductsAdapter(var listProducts: List<Product>, val productLambda: (Product) -> Unit):
    ...
    override fun onBindViewHolder(holder: ProductsViewHolder, position: Int) {
        ...
        holder.cost.text = (listProducts[position].cost / 100).toString() + " р."
        holder.count.text = "Количество: ${listProducts[position].count} шт."

        holder.itemView.setOnClickListener {
            productLambda.invoke(listProducts[position])
        }
    }
